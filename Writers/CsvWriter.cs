...
namespace IqFeed {
	public class CsvWriter : IWriter, IDisposable {
		...
		private void TicksReponseHandler(TicksResponse tick) {
			...
			writer.WriteLine($"{tick.symbol},{tick.timeStamp.UtcValue},{tick.last},{tick.lastSize},{tick.totalVolume},{tick.bid},{tick.ask},{tick.tickID},{tick.basis},{tick.tradeMarketCenter},{tick.tradeConditions}");
		}
